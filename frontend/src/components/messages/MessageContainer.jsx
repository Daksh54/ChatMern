						...
						<div className='flex items-center justify-between gap-4'>
							<div>
								...
								<div className='mt-1 text-xs text-slate-900/80'>
									...
									{isUserTyping(selectedConversation._id) ? " • typing..." : ""}
								</div>
							</div>
							<div className='text-right text-xs text-slate-900'>
								<div className='font-semibold uppercase tracking-wide'>Conversation Pulse</div>
								<div>
									{loadingInsights
										? "Analyzing..."
										: `${conversationInsights?.sentiment?.label ?? "neutral"} (${conversationInsights?.sentiment?.score ?? 0})`}
								</div>
							</div>
						</div>
					</div>
